...
const listBasketItems = () => {
  ...
  basketListEl.innerHTML = basketListHtml
    ? basketListHtml
    : `<li class="basket__item">Sepetin boş hadi sevdiğin kitapları keşfet.</li>`;
  ...
  if (findedBook) {
    ...
    if (basketAlreadyIndex == -1) {
      ...
      if (
        ...
        toastr.error("Üzgünüz stoklarımızda kalmadı.");
        return;
      }
    }
    listBasketItems();
...
const increaseItemToBasket = (bookId) => {
  ...
  if (findedIndex != -1) {
    if (
      ...
      basketList[findedIndex].quantity += 1;
    else toastr.error("Üzgünüz stoklarımızda kalmadı.");
    listBasketItems();
  }
};
